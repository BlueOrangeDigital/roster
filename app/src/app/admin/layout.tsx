...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-cream-50">
      {/* Sidebar */}
      <aside className="w-[240px] bg-navy-950 text-white flex flex-col shrink-0 border-r border-navy-800/60">
        {/* Logo */}
        <div className="px-5 py-5 border-b border-navy-800/60">
          <Link href="/admin" className="flex items-center gap-2.5 group">
            ...
            <div>
              <span className="text-sm font-bold tracking-tight block leading-tight">Roster</span>
        ...
        <nav className="flex-1 px-3 py-4 space-y-px">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={`w-[17px] h-[17px] shrink-0 ${active ? "text-orange-400" : ""}`} />
                {item.label}
              </Link>
            );
          })}
        ...
        <div className="px-3 py-4 border-t border-navy-800/60">
          <button
            ...
            className="flex items-center gap-3 px-3 py-2 rounded-md text-[13px] font-medium text-navy-500 hover:text-navy-200 hover:bg-white/[0.04] w-full transition-colors"
          >
